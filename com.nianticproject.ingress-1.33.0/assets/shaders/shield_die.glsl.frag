...
float tmpvar_6;
  tmpvar_6 = dot (u_contributionsAndAlpha.xyz, texture2D (u_levelsTexture, v_texCoord0).xyz);
float tmpvar_7;
  tmpvar_7 = min (1.0, abs(((tmpvar_5.x - u_rampTargetInvWidth.x) * u_rampTargetInvWidth.y)));
vec3 tmpvar_8;
  tmpvar_8 = (vec3(0.93, 0.11, 0.14) * (tmpvar_5.y + 0.75));
float tmpvar_9;
  if ((tmpvar_7 > 0.5)) {
    tmpvar_9 = tmpvar_5.y;
  } else {
...
vec4 tmpvar_13;
  ...
  tmpvar_13.w = (tmpvar_12.w * u_contributionsAndAlpha.w);
